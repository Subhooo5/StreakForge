...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (normalizedView === 'spotlight') {
      ...
      if (!spotlightUser) {
        const validationErr = new Error('Invalid user for the spotlight view');
        validationErr.name = 'ValidationError';
        throw validationErr;
      }
      ...
      try {
        userRepos = await fetchUserRepos(spotlightUser, {
      ...
      const pushTime = (r: GitHubRepo) => new Date(r.pushed_at ?? r.updated_at ?? 0).getTime();
      const ownRepos = userRepos.filter((r) => !r.fork).sort((a, b) => pushTime(b) - pushTime(a));

      const spotlightCacheControl = isRefreshRequested
      ...
      if (format === 'json') {
        const payload = JSON.stringify({
          ...
          repos: ownRepos.map((r) => ({
            ...
            url: `https://github.com/${r.owner?.login ?? spotlightUser}/${r.name}`,
          })),
        });
        ...
        return new NextResponse(payload, {
          headers: {
            ...
            'Cache-Control': spotlightCacheControl,
            'X-Request-ID': requestId,
          },
        });
      }
      ...
      const selectedRepo = repo
        ? ownRepos.find((r) => r.name.toLowerCase() === repo.toLowerCase())
        : ownRepos[0];

      if (!selectedRepo) {
        const spotlightErrTheme = resolveErrorTheme(searchParams);
        const missingSvg = buildInlineErrorSVG(
          repo ? `Repository "${repo}" not found` : 'No public repositories found',
          {
            bg: spotlightErrTheme.bg,
            accent: spotlightErrTheme.accent,
            text: spotlightErrTheme.text,
            radius: spotlightErrTheme.radius,
          }
        );
        return new NextResponse(missingSvg, {
          status: 404,
          headers: {
            'Content-Type': 'image/svg+xml',
            'Cache-Control': 'no-store',
      ...
      let spotlightSvg = generateRepoSpotlightSVG(selectedRepo, params);
      if (minify) {
        spotlightSvg = optimizeSVG(spotlightSvg);
      }

      return new NextResponse(spotlightSvg, {
        headers: {
          'Content-Type': 'image/svg+xml',
          'Cache-Control': spotlightCacheControl,
          'Content-Security-Policy': SVG_CSP_HEADER,
    ...
    if (format === 'json') {
      ...
      if (primaryUser) {
        try {
          const p = await fetchUserProfile(primaryUser);
          profile = {
            login: p.login,
            name: displayName(p),
            avatar: p.avatar_url,
    ...
    if (format === 'png') {
      ...
      return new NextResponse(new Uint8Array(pngBuffer), {
        headers: {
          ...
          'Cache-Control': cacheControl,
          'X-StreakForge-Grace-Applied': String(grace),
          ETag: weakEtag,
...
function buildErrorResponse(
  ...
  if (isRateLimit) {
    ...
    if (isCircuitOpen) {
      headers['X-StreakForge-Circuit-Status'] = 'Open';
      headers['X-StreakForge-Circuit-Reset-In'] = String(telemetry.resetInMs);
    }
    if (requestId) {
