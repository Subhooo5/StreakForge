...
- **Top-level `/components`** (sibling to `app/`, not nested inside it) holds ONLY components reused across 2+ pages — currently `Navbar.tsx`, `Footer.tsx`, `Hover.tsx`, `Avatar.tsx`, `ContributionHeatmap.tsx`, `LoadingPanel.tsx`. A component used by exactly one page belongs in that page's `app/<feature>/components/`, not here.
- **`content/`** — top-level content modules that are data, not components. Currently `content/docs.ts`, the single source for every word on `/docs`. Author documentation copy there, never inline in a view.
- **`/docs`** follows the standard route shape (`app/docs/page.tsx` + `app/docs/DocsClient.tsx`) and renders `content/docs.ts`. Its root deliberately omits `overflowX: 'hidden'` so native `#anchor` scrolling and the sticky sidebar work — a documented exception to POST-PORT convention 6, which applies to the six ported mockup pages.
...
4. **Canonical nav/footer links.** Home is the source of truth for shared links; all pages match it: Repo `https://github.com/Subhooo5/StreakForge` (nav "GitHub Repo", mobile menu, footer "Repository"); GitHub profile `https://github.com/Subhooo5` (footer "GitHub" + bottom icon); Discord `https://discordapp.com/users/488670412096667648`; X/Twitter `https://x.com/SiMpL36969`; LinkedIn `https://www.linkedin.com/in/subho1817/`; "Documentation" points at `/docs`, deep-linked per page via the `docsHref` prop on `<Footer />`: `/docs#generator`, `/docs#compare`, `/docs#burnout-analyzer`, `/docs#customization-studio`, and plain `/docs` from Home, Dashboard and Docs itself. External links open in a new tab (`target="_blank" rel="noopener"`). The internal route links (`/generator`, `/compare`, `/burnout-analyzer`, `/customize`, `/dashboard`, `#top`) were already correct.
...
10. **Navbar and Footer are shared components.** `components/Navbar.tsx` and `components/Footer.tsx` are the only header/footer markup in the app; no page may inline its own. Both are unified on Home's version — the public logo SVGs (never inline logo markup) and Home's link set. Each page renders `<Navbar theme={theme} toggleTheme={toggleTheme} active="<key>" />` and `<Footer theme={theme} active="<key>" docsHref="..." />`, passing its existing `useTheme()` values down. **Never let Navbar or Footer call `useTheme()` themselves** — the hook is per-component `useState` with no shared store, so a second instance forks the theme and leaves the page wrapper's `sf dark` class stale on toggle. `active` marks the current nav entry and points the logo at `#top` on Home, `/` elsewhere.
