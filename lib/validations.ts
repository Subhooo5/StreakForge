...
export const burnoutParamsSchema = z.object({
  owner: z
    .string({ error: 'Missing "owner" parameter' })
    .trim()
    .min(1, { message: 'owner is required' })
    .max(39, { message: 'GitHub owner cannot exceed 39 characters' })
    .regex(GITHUB_USERNAME_REGEX, { message: 'Invalid GitHub owner' }),
  repo: z
    .string({ error: 'Missing "repo" parameter' })
    .trim()
    .min(1, { message: 'repo is required' })
    .max(100, { message: 'Repository name cannot exceed 100 characters' })
    .regex(GITHUB_REPO_NAME_REGEX, { message: 'Invalid repository name' }),
  excludeBots: z.string().optional().transform(toBooleanFlag),
});
...
export type BurnoutParams = z.infer<typeof burnoutParamsSchema>;
