...
<text x="${statsX}" y="278" fill="#${text}" font-family="'Inter',sans-serif" font-size="16" font-weight="600">${stats.longestStreak}d</text>`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${Math.round(WIDTH * sf)}" height="${Math.round(HEIGHT * sf)}" viewBox="0 0 ${WIDTH} ${HEIGHT}" role="img" aria-labelledby="sf-clock-title" aria-describedby="sf-clock-desc">
  <title id="sf-clock-title">StreakForge Commit Clock for ${safeUser}</title>
  <desc id="sf-clock-desc">A 24-hour polar ring showing ${safeUser}'s commit frequency by hour of day.</desc>
  <defs>
    <filter id="sf-clock-glow">
      <feGaussianBlur stdDeviation="2" result="blur"/>
      <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
    </filter>
  </defs>
  ${params.hideBackground ? '' : `<rect width="${WIDTH}" height="${HEIGHT}" fill="#${bg}" rx="${rx}"/>`}
  <g filter="url(#sf-clock-glow)">
${segments}  </g>
  ${innerCircle}
  ${cardinalLabels}
  ${statsPanel}
