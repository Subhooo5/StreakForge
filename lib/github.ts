...
async function fetchContributionsUncached(
  ...
  if (!repoContributions || !Array.isArray(repoContributions)) {
    console.warn(
      `[StreakForge API] Empty profile or null repository nodes discovered for user "${username}". Falling back to baseline collection.`
    );
    repoContributions = [];
  }
...
export async function getFullDashboardData(username: string, options: FetchOptions = {}) {
  ...
  const commitsForRepo = (name: string, nameWithOwner?: string) =>
    repoContributions.find(
      (c) =>
        c.repository.name === name ||
        (!!nameWithOwner && c.repository.nameWithOwner === nameWithOwner)
  ...
  reposData.forEach((r) => {
    nodes.push({
      ...
      stats: {
        ...
        commits: commitsForRepo(r.name, `${r.owner?.login ?? profileData.login}/${r.name}`),
      },
    });
  ...
  contributedRepos.forEach((r) => {
    nodes.push({
      ...
      stats: {
        ...
        commits: commitsForRepo(r.name, r.nameWithOwner),
      },
    });
