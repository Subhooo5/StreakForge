...
export default function PRStatusDistribution({ data: propData, username, timeoutMs = 5000 }: PRStatusDistributionProps) {
  ...
  if (loading) {
    return (
      <div data-testid="pending-overlay" className="flex items-center justify-center p-8 rounded-xl bg-white dark:bg-[#0a0a0a] border border-black/10 dark:border-[rgba(255,255,255,0.08)] min-h-55">
  ...
  if (error) {
    return (
      <div data-testid="fallback-error" className="flex items-center justify-center p-8 rounded-xl bg-white dark:bg-[#0a0a0a] border border-red-500/20 min-h-55">
        <div className="flex flex-col items-center gap-3 text-center">
          <AlertCircle className="text-red-500" size={24} />
          <p className="text-sm text-red-500 font-medium">{error}</p>
          <button onClick={loadData} className="flex items-center gap-2 px-3 py-1.5 mt-2 text-xs rounded-md bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors">
            <RefreshCw size={12} />
            Retry
          </button>
        </div>
  ...
  const centerValue = activeMeta ? (baseData.find((d) => d.state === activeFilter)?.value ?? data.totalPRs ?? 0) : (data.totalPRs ?? 0);
  const centerLabel = activeMeta ? activeMeta.label : "Total";
  const centerColor = activeMeta ? activeMeta.color : undefined;

  const filteredPRs = activeFilter && data.prs ? data.prs.filter((pr) => pr.state === activeFilter) : [];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="bg-white dark:bg-zinc-900/50 border border-black/10 dark:border-white/10 rounded-3xl p-6 h-full flex flex-col">
      <div className="mb-2">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">{"Status Distribution"}</h2>
        <p className="text-sm text-gray-500">{activeFilter ? `Showing: ${activeMeta?.label || ""} PRs — click again to reset` : "Breakdown of PR states"}</p>
      </div>

      <div className="flex-1 relative min-h-62.5 flex items-center justify-center">
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
            <Pie data={chartData} cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={5} dataKey="value" stroke="none" onClick={(entry) => handleClick(entry as unknown as Record<string, unknown>)}>
              {chartData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.color} style={{ transition: "opacity 0.2s ease", cursor: "pointer" }} opacity={activeFilter === null || activeFilter === entry.state ? 1 : 0.25} />
              ))}
            </Pie>
            <Tooltip
              contentStyle={{
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-3xl font-bold" style={{ color: centerColor }}>
            <span className={centerColor ? "" : "text-gray-900 dark:text-white"}>{centerValue}</span>
          </span>
          <span className="text-xs font-medium text-gray-500 uppercase tracking-widest mt-0.5">{centerLabel}</span>
        </div>
      </div>
      ...
      {totalCount > 0 && (
        <div className="flex justify-center gap-4 mt-2">
          {chartData.map((item) => (
            <button key={item.name} onClick={() => handleClick(item)} tabIndex={-1} className={`flex items-center gap-2 rounded-full px-2 py-1 transition-all duration-200 focus:outline-none ${activeFilter === item.state ? "opacity-100" : "opacity-60 hover:opacity-100"}`} aria-pressed={activeFilter === item.state} aria-label={`Filter by ${item.name} PRs`}>
              <div
      ...
      <AnimatePresence>
        {activeFilter && filteredPRs.length > 0 && (
          <motion.div key={activeFilter} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden mt-4">
            <div className="border-t border-black/10 dark:border-white/10 pt-4 flex flex-col gap-2 max-h-48 overflow-y-auto pr-1">
              {filteredPRs.map((pr) => (
                <a key={pr.url} href={pr.url} target="_blank" rel="noopener noreferrer" className="flex items-start justify-between gap-2 rounded-xl px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                  <div className="flex flex-col min-w-0">
                    <span className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{pr.title}</span>
