...
export default function PRSizeDistribution({ data }: { data: PRInsightData }) {
  ...
  const chartData = [
    ...
    {
      name: "Standard (100–500 LOC)",
      value: dist.standard,
      color: "#3b82f6",
    },
    {
      name: "Massive (>500 LOC)",
      value: dist.massive,
      color: "#ef4444",
    },
  ];
  ...
  return (
    <div className="bg-white dark:bg-zinc-900/50 border border-black/10 dark:border-white/10 rounded-3xl p-6 h-full flex flex-col justify-between">
      <div className="mb-4">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">{"PR Size Distribution"}</h2>
        <p className="text-sm text-gray-500">{"Breakdown by total lines changed"}</p>
      </div>

      <div className="h-[220px] w-full">
        {visibleData.length === 0 ? (
          <div className="flex items-center justify-center h-full text-sm text-gray-400">{"No PR size data available"}</div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={visibleData} layout="vertical" margin={{ top: 10, right: 24, left: 10, bottom: 0 }}>
