...
const MetricCard = ({ title, value, icon: Icon, trend, delay = 0, suffix = "" }: MetricCardProps) => {
  ...
  return (
    ...
    >
      ...
      {trend && <div className="relative z-10 text-xs font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 w-fit px-2.5 py-1 rounded-full">{trend}</div>}
    </motion.div>
  );
};
...
export default function TopMetricsRow({ data }: { data: PRInsightData }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <MetricCard title={"Total PRs"} value={data.totalPRs} icon={GitPullRequest} trend={data.weeklyActivity?.length > 0 ? `+${data.weeklyActivity[data.weeklyActivity.length - 1].prs} this week` : undefined} delay={0.1} />

      <MetricCard title={"Merge Rate"} value={data.mergeRate.toFixed(1)} suffix="%" icon={GitMerge} delay={0.2} />

      <MetricCard title={"Avg Cycle Time"} value={data.avgCycleTime.toFixed(1)} suffix={"hrs"} icon={Clock} delay={0.3} />

      <MetricCard title={"First Review"} value={data.avgTimeToFirstReview.toFixed(1)} suffix={"hrs"} icon={Activity} delay={0.4} />
    </div>
  );
}
