...
export default function RepoPerformanceTable({ data }: { data: PRInsightData }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.9 }} className="bg-white dark:bg-zinc-900/50 border border-black/10 dark:border-white/10 rounded-3xl p-6 flex flex-col overflow-hidden">
      <div className="mb-6">
            ...
            {repoPerformance.map((repo) => (
              <tr key={repo.name} className="group hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                <td className="py-3 pr-4">
                  ...
                  <div className="text-xs text-gray-500 truncate max-w-[150px] sm:max-w-[200px]">{repo.name.split("/")[0]}</div>
                </td>
                <td className="py-3 px-4 text-right font-medium text-gray-700 dark:text-gray-300">{repo.totalPRs}</td>
                <td className="py-3 px-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <div className="w-12 h-1.5 bg-gray-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                      <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${repo.mergeRate}%` }} />
                    </div>
                  </div>
                </td>
                <td className="py-3 pl-4 text-right font-medium text-gray-700 dark:text-gray-300">{repo.reviewCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
