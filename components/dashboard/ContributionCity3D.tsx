...
export default function ContributionCity3D({ data, theme = "dark", days = 98, timeLapseMode = false }: ContributionCity3DProps) {
  ...
  const draw = useCallback(() => {
    ...
    for (const cube of sorted) {
      ...
      const topColor = cube.count === 0 ? darken(palette.bg, -20) : lighten(palette.top, intFactor * 80);
      const leftColor = cube.count === 0 ? darken(palette.bg, -10) : darken(palette.side, -intFactor * 30);
      const rightColor = cube.count === 0 ? darken(palette.bg, -5) : darken(palette.side, intFactor * 20);

      const drawFace = (p0: { cx: number; cy: number }, p1: { cx: number; cy: number }, p2: { cx: number; cy: number }, p3: { cx: number; cy: number }, color: string) => {
        ctx.beginPath();
  ...
  if (hasWebGLFailed) {
    return (
      <div className="flex h-90 w-full items-center justify-center rounded-xl" style={{ background: palette.bg }}>
  ...
  return (
    <div className="relative w-full" style={{ background: palette.bg, borderRadius: 12 }}>
      ...
      >
        <div ref={containerRef} className="w-full" style={{ height: 360, cursor: isDragging ? "grabbing" : "grab" }}>
          <canvas ref={canvasRef} className="w-full h-full" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerLeave={() => setTooltip(null)} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onWheel={onWheel} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} style={{ display: "block" }} />
        </div>
      </ErrorBoundary>
      ...
      {/* Controls hint */}
      {!timeLapseMode && (
        <div className="absolute bottom-3 right-4 text-xs opacity-40 select-none pointer-events-none" style={{ color: palette.accent }}>
      ...
      {timeLapseMode && (
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
          {/* Playback Controls */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl backdrop-blur-md shadow-sm border border-white/10" style={{ background: `${palette.bg}88` }}>
            <button
              onClick={() => {
              ...
              className={`p-1.5 rounded-lg transition-colors text-white ${isExporting ? "opacity-50 cursor-not-allowed" : "hover:bg-white/10"}`}
            ...
            <div className="text-xs font-semibold px-1 min-w-20" style={{ color: palette.accent }}>
              {playbackIndex > 0 &&
                (() => {
                  const recent = data.slice(-days);
            ...
            {/* Export button */}
            <div className="w-px h-5 bg-white/20 mx-1" />
            <button onClick={handleExport} disabled={isExporting} className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg transition-colors text-xs font-medium text-white ${isExporting ? "bg-white/20 cursor-wait" : "hover:bg-white/10"}`} title="Export as WebM video">
              {isExporting ? (
                <>
